...
## portal-config.js (every page includes it FIRST — as `/portal-config.js?v=10`)
All 123 pages reference `?v=10` (cache-bust; ?v=6 forced the `.ml-back` styling,
?v=7–9 the animated wait mark, ?v=10 the status-bar cap). If a portal-config
change must reach them again, bump to ?v=11 across all pages with sed — and check the
