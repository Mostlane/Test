...
async function trackJobTime(env, tid, actor, before, after) {
  try {
    ...
    if (bs === as) return;
    const engs = Array.isArray(after.assignedEngineers) && after.assignedEngineers.length ? after.assignedEngineers : after.assignedTo ? [after.assignedTo] : [];
    const normId2 = (s) => String(s || "").toLowerCase().replace(/\s+/g, ".").trim();
    let mine = engs.some((e) => normId2(e) === normId2(actor));
    if (!mine) {
      try {
        ...
        mine = engs.some((e) => map[normId2(e)] === actor);
      } catch {
      }
    }
    if (!mine) return;
...
async function jobTimeAuto(env, tid, username, monday) {
  ...
  for (const [date, o] of Object.entries(out)) {
    shaped[date] = {
      start: lTime(new Date(o.s).toISOString()),
...
async function handle8(request, env, ctx, url, sess) {
  ...
  if (subpath.startsWith("/jobs/")) {
    ...
    if (parts[2] === "photo-stage" && method === "POST") {
      ...
      else delete job.photoStages[filename];
      job.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
      await saveJob(env, tenantId, job);
  ...
  if (subpath === "/site/thumb" && method === "POST") {
    ...
    await env.JOB_FILES.put(key + ".thumb", thumb.stream(), { httpMetadata: { contentType: thumb.type || "image/jpeg" } });
    try {
      ctx?.waitUntil(caches.default.delete(new Request(`${url.origin}/sla/site/thumb?key=${encodeURIComponent(key)}`)));
    } catch {
    }
    return jsonResponse({ ok: true }, headers);
  }
...
async function handle13(request, env, ctx, url, sess) {
  ...
  if (path === "/office/timesheet" && request.method === "GET") {
    ...
    const monday = mondayOf2(isDateStr2(url.searchParams.get("week") || "") ? url.searchParams.get("week") : londonDate());
    const days = weekDays2(monday);
    const sunday = days[6];
    const { results } = await db.prepare(
...
async function handle16(request, env, ctx, url, sess) {
  ...
  if (path === "/hs/sign" && method === "POST") {
    ...
    if (!id) return error("Missing id", 400, env, request);
    if (!/^data:image\//.test(signature) || signature.length > 4e5) return error("A signature is required.", 400, env, request);
    ...
    mine.status = "signed";
    mine.signedAt = (/* @__PURE__ */ new Date()).toISOString();
    mine.signature = signature;
    await db.prepare("UPDATE hs_documents SET sign_requests=?, updated_at=? WHERE tenant_id=? AND id=?").bind(JSON.stringify(reqs), mine.signedAt, db.tenantId, id).run();
    if (ctx && ctx.waitUntil && row.created_by && row.created_by !== me) ctx.waitUntil(sendToUser(env, db.tenantId, row.created_by, {
      title: "Risk assessment signed",
      body: `${mine.name || me} has signed ${row.ref || "a risk assessment"}.`,
      url: "/hs-docs.html",
      tag: "hs-signed"
    }));
    return json({ ok: true }, {}, env, request);
  ...
  if (path === "/hs/doc/file-signed" && method === "POST") {
    ...
    if (mine.docKey && mine.docKey !== key) {
      try {
        await env.JOB_FILES.delete(mine.docKey);
      } catch {
      }
    }
    mine.docKey = key;
  ...
  if (path === "/hs/doc/attach" && method === "POST") {
    ...
    await env.JOB_FILES.put(key, file.stream(), {
      httpMetadata: { contentType: file.type || "application/octet-stream" },
      customMetadata: { name: file.name || safe, by: me, at: (/* @__PURE__ */ new Date()).toISOString() }
    });
    const atts = parseArr(row.attachments);
    const meta = { key, name: file.name || safe, type: file.type || "", size: file.size || 0, at: (/* @__PURE__ */ new Date()).toISOString(), by: me };
    atts.push(meta);
    await db.prepare("UPDATE hs_documents SET attachments=?, updated_at=? WHERE tenant_id=? AND id=?").bind(JSON.stringify(atts), (/* @__PURE__ */ new Date()).toISOString(), db.tenantId, id).run();
  ...
  if (path === "/hs/doc/sign-request" && method === "POST") {
    ...
    for (const un of usernames) {
      if (reqs.some((s) => s.username === un && s.status !== "declined")) continue;
      reqs.push({ username: un, name: nameOf[un] || un, status: "pending", requestedAt: (/* @__PURE__ */ new Date()).toISOString(), requestedBy: me });
      added.push(un);
    }
    await db.prepare("UPDATE hs_documents SET sign_requests=?, updated_at=? WHERE tenant_id=? AND id=?").bind(JSON.stringify(reqs), (/* @__PURE__ */ new Date()).toISOString(), db.tenantId, id).run();
    for (const un of added) {
      if (ctx && ctx.waitUntil) ctx.waitUntil(sendToUser(env, db.tenantId, un, {
        title: "Risk assessment to sign",
        body: `Please read and sign ${row.ref || "a risk assessment"}.`,
        url: "/hs-sign.html?id=" + encodeURIComponent(id),
        tag: "hs-sign"
      }));
    }
  ...
  if (path === "/hs/doc/sign-cancel" && method === "POST") {
    ...
    if (removed && removed.docKey) {
      try {
        await env.JOB_FILES.delete(removed.docKey);
      } catch {
      }
    }
    await db.prepare("UPDATE hs_documents SET sign_requests=?, updated_at=? WHERE tenant_id=? AND id=?").bind(JSON.stringify(reqs), (/* @__PURE__ */ new Date()).toISOString(), db.tenantId, id).run();
...
async function handle17(request, env, ctx, url, sess) {
  ...
  if (path === "/vancheck/week" && method === "GET") {
    ...
    const week = mondayOf3(wk && /^\d{4}-\d{2}-\d{2}$/.test(wk) ? wk : londonDate2());
    const s = await getSettings(db);
    const { results: drivers } = await db.prepare(
...
var CHASE_LEAD_MS = 2 * 60 * 60 * 1e3;
async function readVanSettings(env, tid) {
  try {
    const row = await env.DB.prepare("SELECT value FROM app_config WHERE tenant_id=? AND key=?").bind(tid, SETTINGS_KEY2).first();
...
async function sendWeeklyReminders(env, now = /* @__PURE__ */ new Date()) {
  ...
  const lonDow = d.toLocaleDateString("en-GB", { timeZone: "Europe/London", weekday: "short" });
  const week = mondayOf3(lonDate);
...
async function handle21(request, env, ctx, url, sess) {
  ...
  if (sub === "/odometer" && method === "POST") {
    ...
    if (!reg || !date || !milesV) return jr3({ error: "reg, date and miles are required" }, headers, 400);
  ...
  if (sub === "/odometer/import" && method === "POST") {
    ...
    return jr3({ ok: true, created, updated, skipped }, headers);
  }
  ...
  if (sub === "/finance" || sub === "/fuel/cards" || sub === "/fuel/entries" || sub === "/fuel/entry" || sub === "/fuel/entry-delete" || sub === "/fuel/stats" || sub === "/fuel/import") {
    if (sub === "/finance" && method === "POST") {
      ...
      return jr3({ ok: true, finance: clean }, headers);
    }
    if (sub === "/fuel/cards" && method === "GET") {
      ...
      return jr3({ ok: true, cards }, headers);
    }
    if (sub === "/fuel/entries" && method === "GET") {
      ...
      const entries = (rows || []).map((r) => ({
        ...
        card: r.card,
        username: r.username,
        name: (byCard[r.card] || {}).name || r.username || "",
        date: r.date || "",
      ...
      return jr3({ ok: true, entries }, headers);
    }
    ...
    if (sub === "/fuel/stats" && method === "GET") {
      ...
      if (card) {
        ...
        for (const k of Object.keys(fuelV)) regsInScope.add(k);
      }
      let miles = 0;
      for (const k of regsInScope) if (odoV[k]) miles += odoV[k].milesDriven;
      const spanDays = first && last ? Math.max(1, (Date.parse(last) - Date.parse(first)) / 864e5) : 0;
  ...
  if (sub === "/handover/template" && method === "GET") {
    return jr3({ ok: true, template: await handoverTemplate(env, tid), defaults: DEFAULT_HANDOVER }, headers);
  }
  if (sub === "/handover/template" && method === "POST") {
    ...
    return jr3({ ok: true, template: tpl }, headers);
  }
...
async function fuelByVehicle(env, tid) {
  ...
  try {
    ...
    for (const e of results || []) {
      ...
      o.litres += Number(e.litres) || 0;
      o.spend += Number(e.cost) || 0;
      o.count++;
...
async function mpgByVehicle(env, tid) {
  ...
  for (const k of Object.keys(odo)) {
    ...
    let litres = 0;
    for (const e of rowsByReg[k] || []) if (e.date > o.first && e.date <= o.last) litres += e.litres;
...
function financeOf(v) {
  return parseJson(v && v.finance, {}) || {};
}
function runningCost(fin, fuelV, odoV, maint12) {
  ...
  if (fuelV && fuelV.spend > 0 && fuelV.first && fuelV.last) {
    ...
    fuelYear = fuelV.spend / days * 365;
    fuelProjected = days < 365;
  }
  ...
  return {
    ...
    roadTax,
    finance,
    fuel: Math.round(fuelYear),
    maintenance,
    excess: Math.round(excess),
    milesYear: Math.round(milesYear),
    total: Math.round(total),
    projected: fuelProjected || excessProjected,
    fuelProjected,
    excessProjected
  };
}
function periodStats(spanDays, totals) {
  const per = (days) => {
    ...
    v.litres = Math.round(v.litres * 10) / 10;
    v.miles = Math.round(v.miles);
    return v;
  };
...
var GROUPS_KEY = "chat_groups";
var DEFAULT_GROUPS = [{ id: "office", name: "Office", members: ["Joanna", "Tanya", "Megan", "Chloe"] }];
var _grpCache = { at: 0, tid: null, groups: null };
async function loadGroups(env, tid) {
  if (_grpCache.groups && _grpCache.tid === tid && Date.now() - _grpCache.at < 12e4) return _grpCache.groups;
...
function groupById(groups, id) {
  return groups.find((g) => lc(g.id) === lc(id)) || null;
}
function isMember(group, user) {
  return !!(group && (group.members || []).some((m) => lc(m) === lc(user)));
}
async function groupUnreadCount(env, tid, me) {
  ...
  for (const g of groups) {
    ...
    const readMap = {};
    reads.forEach((rr) => {
      readMap[lc(rr.thread_key)] = rr.last_id;
    });
    rows.forEach((m) => {
      if (m.id > (readMap[lc(m.thread_key || "")] || 0)) n++;
    });
  }
  return n;
}
async function groupThreads(env, tid, me) {
  ...
  for (const g of groups) {
    ...
    Object.values(byKey).forEach((t) => {
      out.push({ isGroup: true, group: g.id, key: t.key, with: member ? g.name + " \xB7 " + t.key : g.name, last: t.last, at: t.at, unread: t.unread });
    });
  }
  return out;
}
async function handle22(request, env, ctx, url, sess) {
  ...
  if (sub === "/groups" && method === "GET") {
    const groups = await loadGroups(env, tid);
    return jr4({ ok: true, groups: groups.map((g) => ({ id: g.id, name: g.name, members: g.members })) }, headers);
  }
  ...
  if (sub === "/thread" && method === "GET" && url.searchParams.get("group")) {
    ...
    if (!g) return jr4({ error: "unknown group" }, headers, 400);
    const member = isMember(g, me);
    let key = (url.searchParams.get("key") || "").trim();
    if (!member) key = me;
    if (!key) return jr4({ error: "key required" }, headers, 400);
    ...
    return jr4({ ok: true, isGroup: true, group: g.id, key, name: g.name, members: g.members, messages }, headers);
  }
  ...
  if (sub === "/send" && method === "POST") {
    ...
    if (groupId) {
      if (!body) return jr4({ error: "body required" }, headers, 400);
      ...
      if (!member) key = me;
      else if (!key) return jr4({ error: "key required" }, headers, 400);
...
async function handle23(request, env, ctx, url, sess) {
  ...
  if (sub === "/status" && method === "GET") {
    ...
    const ackMap = {}, docMap = {}, ipMap = {};
    acks.forEach((a) => {
      ackMap[lc2(a.username)] = a.signed_at;
      if (a.doc_key) docMap[lc2(a.username)] = a.doc_key;
      if (a.ip) ipMap[lc2(a.username)] = a.ip;
    });
    const users = await recipientUsers(env, tid, memo);
    const signed = [], unsigned = [];
    users.forEach((u) => {
      if (ackMap[lc2(u.username)]) signed.push({ username: u.username, name: fullName(u), at: ackMap[lc2(u.username)], ip: ipMap[lc2(u.username)] || null, docKey: docMap[lc2(u.username)] || null });
...
async function handle24(request, env, ctx, url, sess) {
  ...
  if (path === "/costing/job-pos" && method === "GET") {
    ...
    const pos = rows.map((r) => {
      ...
      if (priced) {
        total += v;
        if (String(r.cost_category || "").toLowerCase() === "subcontractor") subcontractor += v;
        else materials += v;
      } else unpriced++;
      return {
        supplier: r.supplier || "",
        ...
        incident: r.incident_no || r.job_ref || "",
        by: r.engineer_name || "",
        date: r.d || ""
      };
    });
    ...
    return json({
      ...
      subcontractor: round(subcontractor),
      unpriced,
      pos
    }, {}, env, request);
  }
...
async function poRows(env, from, to) {
  ...
  try {
    const { results } = await env.PO_DB.prepare(
      "SELECT engineer_name, site, supplier, cost_ex_vat, incident_no, cost_category, trade, substr(issued_at,1,10) AS d FROM po_log WHERE (deleted IS NULL OR deleted=0) AND substr(issued_at,1,10) BETWEEN ? AND ?"
    ).bind(from, to).all();
    return results || [];
  } catch {
    return [];
...
async function ensure5(env) {
  ...
  try {
    await env.DB.prepare("ALTER TABLE compliance_files ADD COLUMN scheme TEXT NOT NULL DEFAULT 'coop'").run();
  } catch {
  }
  try {
    await env.DB.prepare("CREATE INDEX IF NOT EXISTS idx_compfiles_code ON compliance_files(tenant_id, scheme, code, type)").run();
  } catch {
  }
  try {
  ...
  try {
    await env.DB.prepare("ALTER TABLE compliance_stores ADD COLUMN scheme TEXT NOT NULL DEFAULT 'coop'").run();
  } catch {
  }
  try {
...
async function bumpDue(env, tid, scheme, code, type, dateStr) {
  ...
  try {
    ...
    next = addYears(dateStr, DUE_YEARS[type] || 1);
  }
  if (!next) return;
  const at = (/* @__PURE__ */ new Date()).toISOString();
...
async function handle25(request, env, ctx, url, sess) {
  ...
  if (sub === "/file" && method === "POST") {
    ...
    if (form.get("bump") && docDate) {
      try {
        await bumpDue(env, tid, scheme, code, type, docDate);
      } catch {
      }
    }
...
async function handle26(request, env, ctx, url, sess) {
  ...
  try {
    ...
    if (path === "/api/pos" && method === "POST") {
      ...
      if (office) {
        b.source = "office";
        b.office_user_slug = userSlug(sess);
        b.office_user_name = userName(sess);
      } else {
        b.source = "engineer";
        b.engineer_slug = userSlug(sess);
...
function cleanData(raw) {
  ...
  try {
    ...
    return { error: "Bad programme data" };
  }
...
async function getShare(db, token) {
  ...
  if (s.expires_at && new Date(s.expires_at) < /* @__PURE__ */ new Date()) return { error: "This link has expired. Ask Mostlane for a new one.", code: 410 };
  return { share: s };
}
...
async function handle29(request, env, ctx, url) {
  ...
  if (method === "POST" && pathname === "/prog/shared/open") {
    ...
    ).bind((/* @__PURE__ */ new Date()).toISOString(), share.token, db.tenantId).run());
    let data = {};
    try {
  ...
  if (method === "POST" && pathname === "/prog/shared/suggest") {
    ...
    if (claimed) {
      const known = await db.prepare("SELECT rev FROM programme_revisions WHERE prog_id=? AND tenant_id=? AND rev=?").bind(share.prog_id, db.tenantId, claimed).first();
  ...
  if (method === "GET" && pathname === "/prog/list") {
    ...
    const revBy = {};
    for (const r of revs) revBy[r.prog_id] = { rev: r.rev, issuedAt: r.issued_at };
    const sBy = {};
