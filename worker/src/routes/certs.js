...
const FYR_STAGES = ["tested", "to_review", "quoted", "ordered", "in_works", "done", "invoiced"];
...
// The completion day (YYYY-MM-DD) of a portal job — the last finished statusHistory
// entry, else updatedAt/scheduledAt.
function completedDay(j) {
  let latest = "";
  for (const h of (Array.isArray(j.statusHistory) ? j.statusHistory : [])) {
    if (h && /complete|closed|invoiced/i.test(String(h.status || ""))) { const at = String(h.at || ""); if (at > latest) latest = at; }
  }
  return String(latest || j.completedAt || j.updatedAt || j.scheduledAt || "").slice(0, 10);
}
// The full schedule of 5-year electrical tests completed in `year` — portal
// (elecTest) jobs + the Workever archive — each enriched with its remedials case
// (stage / lines / order) if one exists, plus any Concerto-imported case for the
// year. One row per test; the client sets a stage on any of them.
async function fiveYearSchedule(env, tid, year) {
  const y = String(year || new Date().getFullYear());
  const rows = [];
  const seen = new Set();
  // Existing cases, keyed by id.
  ...
  for (const c of caseRows) {
    ...
    let lines = []; try { lines = JSON.parse(c.lines || "[]"); } catch {}
    caseByKey[c.id] = { row: c, data: d, lines };
  }
  // Self-heal: every completed portal elec test with remedials should have a case
  // (covers ones finished before the auto-open hook) — silent, no notification.
  const jobs = await listJobs(env, tid);
  for (const j of jobs) {
    if (j && j.elecTest && Array.isArray(j.remedials) && j.remedials.some(r => r && (r.description || (r.photos || []).length)) &&
        /complete|closed|invoiced/i.test(String(j.status || "")) && completedDay(j).slice(0, 4) === y && !caseByKey["JOB-" + j.id]) {
      try { await upsertFiveYearFromJob(env, tid, j, { silent: true }); } catch {}
    }
  }
  // Re-read cases (a self-heal may have added some).
  const caseRows2 = (await env.DB.prepare("SELECT * FROM five_year_remedials WHERE tenant_id=?").bind(tid).all().catch(() => ({ results: [] }))).results || [];
  const caseByKey2 = {};
  for (const c of caseRows2) { let d = {}; try { d = JSON.parse(c.data || "{}"); } catch {} let lines = []; try { lines = JSON.parse(c.lines || "[]"); } catch {} caseByKey2[c.id] = { row: c, data: d, lines }; }
  // 1) Portal completed elec tests this year.
  for (const j of jobs) {
    if (!j || !j.elecTest) continue;
    if (!/complete|closed|invoiced/i.test(String(j.status || ""))) continue;
    const day = completedDay(j); if (day.slice(0, 4) !== y) continue;
    const key = "JOB-" + j.id; const c = caseByKey2[key];
    ...
    rows.push({
      key, source: "portal", jobId: j.id, store_code: fyrCode(j.siteCode) || String(j.siteCode || "").trim(),
      siteName: j.siteName || j.helpdeskRef || j.reference || "", engineer: (Array.isArray(j.assignedEngineers) ? j.assignedEngineers[0] : "") || "",
      completedAt: day, remCount: rems.length, lines, worksJobId: (c && c.data.worksJobId) || j.remedialsWorksJobId || "",
      stage: (c && c.row.stage) || (rems.length ? "to_review" : "tested"),
    });
    seen.add(key);
  }
  // 2) Workever archive tests completed this year (Chapplins excluded).
  const arch = (await env.DB.prepare(
    "SELECT id, ref, site_code, status, completed_at, json_extract(data,'$.jobName') AS jobName, json_extract(data,'$.siteName') AS siteName, json_extract(data,'$.customerName') AS customerName " +
    "FROM sla_jobs_archive WHERE tenant_id=? AND completed_at>=? AND completed_at<? AND id NOT LIKE 'CHAP-%' " +
    "AND (lower(search) LIKE '%5 year%' OR lower(search) LIKE '%fixed wire%' OR lower(search) LIKE '%eicr%' OR lower(search) LIKE '%electrical install% condition%')"
  ...
  for (const r of arch) {
    ...
    const key = "ARCH-" + r.id; if (seen.has(key)) continue; const c = caseByKey2[key];
    let nm = r.siteName || r.customerName || "";
    if (!nm && r.jobName) { const p = String(r.jobName).split(" - ").map(s => s.trim()).filter(Boolean); nm = p.length >= 2 ? p[1] : (p[0] || ""); }
    rows.push({
      key, source: "archive", jobId: "", archiveId: r.id, store_code: fyrCode(r.site_code) || "",
      siteName: String(nm || "").slice(0, 200) || ("Store " + (r.site_code || "")), engineer: "",
      completedAt: String(r.completed_at || "").slice(0, 10), remCount: c ? c.lines.length : 0, lines: c ? c.lines : [],
      worksJobId: (c && c.data.worksJobId) || "", stage: (c && c.row.stage) || "tested",
    });
    seen.add(key);
  }
  // 3) Concerto-imported / other cases with a quote date this year not already shown.
  for (const c of caseRows2) {
    if (seen.has(c.id)) continue;
    if (String(c.data && c.data.source) === "job") continue;   // JOB-* already covered
    const day = String(c.row.quote_date || c.row.updated_at || "").slice(0, 10);
    if (day.slice(0, 4) !== y) continue;
    rows.push({
      key: c.id, source: "concerto", jobId: "", store_code: c.row.store_code || "",
      siteName: c.row.site_name || "", engineer: "", completedAt: day, remCount: c.lines.length, lines: c.lines,
      worksJobId: (c.data && c.data.worksJobId) || "", stage: c.row.stage || "quoted", sr: c.row.sr || "",
    });
    seen.add(c.id);
  }
  await attachRemedialOrders(env, tid, rows);   // sets r.order by store code / sr
...
export async function handle(request, env, ctx, url, sess) {
  ...
  // GET /certs/five-year/schedule?year= — the full completed-test schedule.
  ...
  // POST /certs/five-year/stage {id, stage, meta?} — set a test/remedial's stage.
  // Upserts a stage-only row when the id isn't an existing case (a satisfactory
  // portal test / an archive test being tracked for the first time).
  if (sub === "/five-year/stage" && method === "POST") {
    ...
    const now = new Date().toISOString();
    const upd = await env.DB.prepare("UPDATE five_year_remedials SET stage=?, updated_at=? WHERE tenant_id=? AND id=?")
      .bind(stage, now, tid, id).run();
    if (!upd.meta || !upd.meta.changes) {
      ...
      ).bind(id, tid, "", fyrCode(m.storeCode) || String(m.storeCode || ""), String(m.siteName || "").slice(0, 200), "5-year electrical test",
        String(m.completedAt || now).slice(0, 10), 0, "", "", stage, "[]", JSON.stringify(data), now, now).run();
    }
    _fyrTestedCache = { tid: null, at: 0, map: null };
