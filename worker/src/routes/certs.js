...
export async function handle(request, env, ctx, url, sess) {
  ...
  if (sub === "/jobs/bulk-create" && method === "POST") {
    ...
    for (const it of sites) {
      ...
      const setNum = (type === "em" && emKind !== "monthly") ? await emSetFor(env, tid, num) : "";
      const desc = type === "em"
        ? (emKind === "monthly" ? `Monthly emergency lighting flick test — ${s.site_name || num}` : `Import certificate number ${setNum || num}-${yr}`)
        : `Import certificate number ${num}-${yr}`;
      ...
      const job = await createOrUpdateJobFromPayload(env, tid, {
        ...
        emTest: type === "em", pat: type === "pat", emKind,
