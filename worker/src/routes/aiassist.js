...
async function toolPlanEmpat(env, tid, caps, args) {
  ...
  // A ready-to-embed engineer briefing (goes into each job's description).
  const briefing = "Interleaved EM/PAT plan (" + hm(plan.startMin) + "–" + hm(plan.endMin) + "):\n" + timeline.map(t => "• " + t).join("\n");
  return {
    date, count: sites.length, briefing,
    ...
    for (const j of jobs) {
      ...
      if (j.action === "assign") {
        ...
        payload = {
          ...
          ...((j.emTest || j.pat || j.electrical || j.firestopping)
